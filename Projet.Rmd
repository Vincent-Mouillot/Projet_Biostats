...
```{r}
stepAIC(object = glm(formula = Malade ~ Sexe * Eau * Boeuf * Oeuf * Age * Eclair,
    family = "binomial", 
    data = Intoxications),
    direction = "backward")
```
...
Intoxications <- Intoxications %>% 
  mutate(Symptomes = case_when(
    ...
    Symptomes %in% c(3, 4) ~ "Très malade",
    TRUE ~ ""
  ))
...
Changer ordre levels "sains, "peu malade" et "tres malade" avec sain en reference
